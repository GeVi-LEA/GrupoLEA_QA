...
(function ($) {
	...
	Plugin.prototype._init = function (target, settings) {
		...
		if (settings["allow_upload"]) {
			...
			$('<a class="btn btn-success upload" />')
				...
				.append(
					$('<input type="file" name="files[]" multiple="">').change(function () {
			...
			if (plugin._enableDragDrop) {
				...
				$dragReceiver
					.bind(plugin._namespaceEvents(["drop", "dragenter", "dragover"]), function (e) {
			...
			plugin._request(
				...
				function () {
					...
					$files
						...
						.append(
							$('<a href="#" />')
								.text(plugin._options["strings"]["try_again"])
								.click(function () {
		...
		ajax = $.extend(ajax, {
			...
			error: function (jqXHR) {
				...
				if ($file_elm) {
					...
					var $message = $("<p />").append($('<span class="label label-danger" />').text(message)).hide().prependTo(plugin.$fm).fadeIn();
	...
	Plugin.prototype._prepareFileDrag = function ($element, destFolder) {
		...
		if (plugin._enableDragDrop && plugin._options["allow_folders"]) {
			$element
				.bind("drop dragenter dragover", function (e) {
					var dataTransfer = e.originalEvent && e.originalEvent.dataTransfer;
	...
	Plugin.prototype._createFile = function (fileData) {
		var plugin = this;
		var $file_elm = $('<div class="file" />').data("file", fileData);
		...
		if (fileData["is_folder"]) {
			...
			onclick = function () {
				...
				var fileUrl =
					plugin._options["ajax_endpoint"] +
					(plugin._options["ajax_endpoint"].indexOf("?") == -1 ? "?" : "&") +
					"action=" +
					(download ? "download" : "show") +
					"&file=" +
					fileData["name"] +
					"&folder=" +
					plugin._currentFolder;
		...
		$('<div class="icon" />')
			...
			.append(
				$("<img />")
					...
					.attr("src", fileData["icon"] || src)
					.bind("error", function (event) {
						$(this)
							.unbind(event)
							.attr("src", plugin._options["icons_url"] + "/unknown.png");
					})
			)
			.appendTo($file_elm);
		...
		if (fileData["uploading"]) {
			//Cancel upload
			$('<a class="btn btn-sm btn-warning cancel" />').attr("title", plugin._options["strings"]["cancel_upload"]).prepend('<i class="icon-cancel"></i>').appendTo($tools);
		...
		//Drag & drop
		if (plugin._options["allow_folders"]) {
			$file_elm.attr("draggable", true).bind("dragstart", function (e) {
				if (!fileData["uploading"]) {
		...
		if (jQuery().modal) {
			...
			var $response =
				promptValue !== false
					...
					: $();
			var $modal = $(
				'<div class="jquery_fm modal fade"> \
...
<div class="modal-body"></div> \
 ...
 <button type="button" class="btn btn-primary" data-dismiss="modal">Save changes</button> \
			...
			var $acceptButton = $modal
				.find(".btn-primary")
				.text(plugin._options["strings"]["accept"])
				.click(function () {
					onOK($response.val());
				});
			...
			$modal
				.find(".btn-default")
				.text(plugin._options["strings"]["cancel"])
				.click(function () {
					if (onCancel) {
						onCancel();
					}
