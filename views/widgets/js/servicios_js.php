...
/* WIDGET SERVICIOS_JS */
const widget_servicios = () => {
    const serv = __url__;
    let clientes_servicios = [];
    let cat_servicios = [];
    let clientes_id_servicios = [];
    ...
    $(document).ready(function() {
        ...
        $('.shawCalRanges').daterangepicker({
            ...
            $("#cmbClientes_servicios").find("option").remove();
            $("#cmbTipoServicios").find("option").remove();
            buscaServicios();
            llenaclientes = 0;
        });
        $("#cmbClientes_servicios").find("option").remove();
        $("#cmbClientes_servicios").select2({
        ...
        // width: 'resolve'
        // });
        $("#cmbClientes_servicios, #cmbTipoServicios").change(function() {
            buscaServicios();
        });
        buscaServicios();
        // getCatServicios();
    ...
    const buscaServicios = () => {
        ...
        jQuery.ajax({
            ...
            data: {
                clientes: $("#cmbClientes_servicios").val().join(", "),
                fechas: $("#fechas-startend").val(),
                tiposervicios: "(SELECT id FROM catalogo_servicios)",
            },
            method: 'POST',
            dataType: "json",
        }).then(resp => {
            ...
            for (x = 0; x < servicios.servicios.length; x++) {
                if (clientes_servicios.indexOf(servicios.servicios[x].nom_cliente) < 0) {
                    clientes_servicios.push(servicios.servicios[x].nom_cliente);
                    clientes_id_servicios.push(servicios.servicios[x].id_cliente);
                }
            ...
            for (l = 0; l < clientes_servicios.length; l++) {
                // console.log(clientes[l]);
                if (llenaclientes == 0) {
                    $("#cmbClientes_servicios").append('<option value="' + clientes_id_servicios[l] + '">' + clientes_servicios[l] + '</option>');
        ...
        myChart.on('click', function(params) {
            ...
            try {
                ...
                if (params.nodeData.dataIndex == 0) {
                    buscaServiciosChart($("#cmbClientes_servicios").val().join(", "));
                }
            }
        });
        swal.close();
